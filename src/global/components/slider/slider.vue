<template>
  <div class="m-slider" ref="slider">
    <div class="slider-wrapper" ref="warpper" :class="direction == 'vertical' ? 'slider-wrapper-vertical' : ''" :style="dragStyleObject">
      <div class="m-slider-item" :style="itemHeight" v-html="lastItem" @click="onLastItemClick"></div>
      <slot></slot>
      <div class="m-slider-item" :style="itemHeight" v-html="firtstItem" @click="onFirstItemClick"></div>
    </div>
    <div v-if="showPagination" class="slider-pagination" :class="direction == 'vertical' ? 'slider-pagination-vertical' : ''">
      <span class="slider-pagination-item" v-for="(t, i) in itemNums" :key="i" :class="paginationIndex == i ? 'slider-pagination-item-active': ''"></span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onFirstItemClick() {
      this.itemsArr[this.itemsArr.length - 1].$el.click()
    },
    onLastItemClick() {
      this.itemsArr[0].$el.click()
    },
    init() {
      this.destroy()
    ...
    touchEndHandler() {
      ...
      if (touches.moveTag == 2) {
        ...
        if (timeDiff > 300 && Math.abs(moveOffset) <= warpperSize * 0.5) {
