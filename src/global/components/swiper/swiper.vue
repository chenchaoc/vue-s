<template>
  <div class="m-swiper" :class="'m-swiper-' + direction" @touchstart.stop="_onTouchstart" @touchmove.stop="_onTouchmove"
    @touchend.stop="_onTouchend">
    <div class="m-swiper-wrapper" ref="swiperWrapper" :style="{ transform: 'translate3d(' + translateX + 'px, '+ translateY +'px, 0)',
          'transition-duration': transitionDuration + 'ms' }" @transitionend.stop.prevent="_onTransitionend">
      <slot></slot>
    </div>
    <div class="m-swiper-pagination" v-if="showPagination">
      <div class="m-swiper-pagination-bullet" v-for="(n, index) of slideCount" :key="index" :class="{ 'm-swiper-pagination-bullet-active': index === activeIndex }"
        @click.stop.prevent="paginationClickable ? _onPaginationClick($event, index) : undefined" v-html="paginationBulletRenderList[index]"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      isSlideChanged: true, // 标明子组件(slide)发生过改变，父组件需要据此重建副本或分页器
  ...
  methods: {
    _init() {
      ...
      if (this.loop) {
        this.isSlideChanged = false
        this._createDuplicateSlide() // 首次
      }
      parseInt(this.autoplay) >= 500 && this.startAutoplay(this.autoplay)
      this.$refs.swiperWrapper.style.height = this.$refs.swiperWrapper.clientHeight + 'px'
      this._events['init'] && this.$emit('init')
    },
    _destroy() {
      this.stopAutoplay()
      if (this.loop && this.duplicateFirstSlideVm && this.duplicateLastSlideVm) {
...
.m-swiper-pagination {
  ...
  .m-swiper-vertical & {
    ...
    width: 16px + 10px * 2; // fixed ios bug, wrap-reverse need
    height: 100%;
    top: 0;
    right: 10px;
  }
